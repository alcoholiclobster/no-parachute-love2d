...
-- MIT License
local ffi = type(_G.jit) == 'table' and _G.jit.status() and require 'ffi'
local vec3, quat
...
quat = {
    ...
    __index = {
        ...
        clone = function(q) return quat(q.x, q.y, q.z, q.w) end,

        unpack = function(q) return q.x, q.y, q.z, q.w end,
        ...
        between = function(u, v) return quat():setBetween(u, v) end,
        ...
        slerp = function(q, r, t, out)
            ...
            if 1 - dot < .0001 then return q:lerp(r, t, out) end
...
return {vec3 = vec3, quat = quat, vector = vec3, rotation = quat}
